...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 22 18:00:40 2022
...
class OutputScoreCAMWhiteningNorm:
    def __init__(self,
                 model,
                 target_layers,
                 use_cuda=False,
...
#                zip(self.target_layers, activations_list, grads_list):
        ...
        ## Whiten activation maps
        all_activations = all_activations - np.mean(all_activations, axis=0)
        print('Performing whitening...')
        pca = PCA()
        all_activations = pca.fit_transform(all_activations)
        
#        print('Computing covariance...')
#        covmat = np.cov(all_activations, rowvar=False)
#        print('Computing eigendecomposition...')
#        w, v = LA.eig(covmat)
#        
#        diagw = np.diag(1/(w+1e-5)**0.5)
#        diagw = diagw.real.round(4)
#        all_activations = np.dot(np.dot(diagw, v.T), all_activations) ## Apply scaling and rotation
#        
#        new_covmat = np.cov(all_activations,rowvar=False)
